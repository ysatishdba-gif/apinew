...
With `TRANSACTION_SELECTION_URL` set, each record type first runs the transaction-selection pipeline (the standalone link-code flow): cluster selection → activities of the probable documents (BigQuery activity table) → Stage 1 activity selection (transaction-selection service, Yes/No) → documents of the selected activities → Stage 2 document selection → document classes of the selected documents (BigQuery results table) → Stage 3 class selection → the selected classes' node ids are the CUIs. Only what that pipeline leaves uncoded goes through the direct path below. Stage counts are logged per record type (`Record-type transaction pipeline`).
...
2. For each name — and for each vocabulary label the model matched it to in the contextual-environment call (`CLUSTER_ALIAS_TEXTS_MAX`, the `/v1` semantic match reused: `PRES. record` is also searched as `Prescription Record`) — `POST CLUSTER_SELECTION_URL` with `{cluster_set, text_list: [text], context_list: [tag names joined by ", " — or the text itself when there are no tags], top_k, top_p, combined}`; every `member_name` in the answer is a probable document. Calls run concurrently.
3. The probable documents, plus the record-type label and its aliases themselves, are matched against the in-memory document catalog (`MEMBER → MAP → CLUSTER`, loaded once per process, `CLUSTER_DOCUMENT_CATALOG_TTL_SECONDS`): normalised equality, punctuation-folded equality, abbreviation-aware token match (`CLUSTER_MEMBER_MATCH_MIN_TOKEN_SCORE` 0.6: prefixes, contractions, plurals, acronyms, reordered qualifiers), then character n-gram similarity (`CLUSTER_MEMBER_MATCH_MIN_SIMILARITY` 0.75) — never exact string equality alone, and no BigQuery call per request (the per-request query remains as the fallback when the catalog cannot load). Each record type's `coding` is the ordered, de-duplicated list of CUIs of *its* documents. A label that names a document class directly is coded from the class label. A name still uncoded keeps the `/v1` vocabulary CUIs (`RECORD_TYPE_JSON_FALLBACK`), and as the last stage is matched by the model against the cluster's document classes in one call per request (`RECORD_TYPE_LLM_FALLBACK`); `coding: []` only when every stage found nothing, logged as `Record types uncoded after every stage`.
...
| `temporal` | first non-default window in pipeline order: vocabulary name, CUIs, formula | **identical** — same projection, same shape, same kind of value |
...
| Record-type names | `/v1` prompt (KNOWN RECORD TYPES block included) | **same prompt, same names**; coding = document cluster, `/v1` vocabulary match as the fallback (`RECORD_TYPE_JSON_FALLBACK`) |
