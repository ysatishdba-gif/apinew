...
from app.utils.record_type_fallback import resolve_with_model
...
# Tag vocabulary for /v2 and /v3 (optional; same GCS-first / local-fallback
# pattern). Not configured -> tags keep an empty coding, exactly as /v1.
tag_vocab = None
try:
    if config.TAG_GCS_BUCKET and config.TAG_GCS_PATH:
        try:
            tag_vocab = ConceptVocab.from_gcs(
                config.PROJECT_ID, config.TAG_GCS_BUCKET, config.TAG_GCS_PATH
            )
        except Exception as gcs_err:
            lg.log_struct(
                message="Failed to load tag vocabulary from GCS; trying local file",
                structured_data={
                    **base_log,
                    "bucket": config.TAG_GCS_BUCKET,
                    "path": config.TAG_GCS_PATH,
                    "error": str(gcs_err),
                    "error_type": type(gcs_err).__name__,
                },
                severity="WARNING",
            ...
            tag_vocab = None
    if tag_vocab is None and config.TAG_LOCAL_PATH:
        tag_vocab = ConceptVocab.from_file(config.TAG_LOCAL_PATH)
    if tag_vocab is not None and len(tag_vocab) == 0:
        tag_vocab = None
    lg.log_struct(
        message=(
            "Tag vocabulary loaded for /v2 and /v3 retrieval signals"
            if tag_vocab is not None
            else "No tag vocabulary configured; tags carry names and topics only"
        ),
        structured_data={
            **base_log,
            "term_count": len(tag_vocab) if tag_vocab is not None else 0,
            "gcs_path": config.TAG_GCS_PATH if config.TAG_GCS_BUCKET else None,
            "local_path": config.TAG_LOCAL_PATH or None,
        },
        severity="INFO",
    )
except Exception as e:
    lg.log_struct(
        message="Tag vocabulary unavailable; tags will have empty coding",
        structured_data={**base_log, "error": str(e), "error_type": type(e).__name__},
        severity="ERROR",
    )
    tag_vocab = None

# Document-cluster diagnostics ("no member names", "names without a BigQuery
# row") go through the service logger.
document_cluster.set_logger(lg, base_log)
...
if document_cluster.is_configured():
    from app.utils import document_transaction_pipeline as _tx
    ...
    lg.log_struct(
        ...
        structured_data={
            **base_log,
            ...
            # Which path codes record types: the transaction-selection
            # pipeline (activities -> documents -> classes, each filtered by
            # the service) or the direct cluster -> catalog lookup.
            "record_type_resolution": (
                "transactions" if _tx.is_enabled() else "cluster"
            ),
            "transaction_selection_url": config.TRANSACTION_SELECTION_URL or None,
            "activity_table": config.CLUSTER_ACTIVITY_TABLE,
            "results_table": config.CLUSTER_ALL_RESULTS_TABLE,
        },
        severity="INFO",
    )
else:
...
@app.post(
    ...
    summary="Extract retrieval signals with canonical temporal matching (indexed vocabulary)",
    description=(
        "Same request, response schema and per-query block as /v1 and /v2 "
        "(Action Finder gate and document-cluster record types as /v2). What changes "
        "is only how the temporal window is MATCHED: the temporal vocabulary is never "
        "sent to the model; the model emits each window as a structured span and code "
        "resolves it to the vocabulary entry that means it (the exact window, else the "
        "narrowest vocabulary window that contains it). `temporal` is the same single "
        "object as /v1 with a vocabulary name, its CUIs and its formula."
    ),
    response_model=RetrievalSignalsResponse,
)
...
async def _run_retrieval_signals(
    ...
    """Shared /v2 and /v3 handler. Same response shape as /v1 for both; /v2 =
    vocab_list temporal matching, /v3 = canonical (index) matching."""
    ...
    async def _process_query(q) -> None:
        ...
        v2_result = await pipeline.run_v2_async(
            ...
            enable_retrieval_signals=True,
            location=resolved_location,
            # The contextual-environment prompt is /v1's, KNOWN RECORD TYPES
            # block included, so record-type NAMES are generated exactly as
            # /v1. Only their coding changes: the document cluster first, the
            # /v1 vocabulary match as the fallback (below).
        ...
        # /v1 projection: same block shape, same temporal choice (first
        # non-default window in pipeline order), same record-type names.
        block = project_query_signals(
            query_id=q.id,
            text=q.text,
            v2_result=v2_result,
            record_type_vocab=record_type_vocab,
            temporal_vocab=temporal_vocab,
            tag_vocab=tag_vocab,  # optional TAG_* vocabulary; None -> as /v1
        )
        vocabulary_coded = {
            rt["name"].lower(): rt["coding"] for rt in block.get("record_types") or []
        }
        ...
        try:
            ...
            cluster_seconds.append(time.perf_counter() - cluster_started)
        # Deterministic fallback: a name the cluster could not code keeps the
        # CUIs the /v1 vocabulary match found for it (identical to /v1 output
        # for that record type), so cluster coverage gaps never blank a code.
        if config.RECORD_TYPE_JSON_FALLBACK:
            for rt in block["record_types"]:
                if not rt.get("coding"):
                    rt["coding"] = vocabulary_coded.get(rt["name"].lower(), [])
        # Model fallback: whatever is still uncoded is matched by the model
        # against the cluster's document classes, in one call for all of them
        # (the /v1 semantic match, against the classes the cluster defines).
        unresolved = [rt for rt in block["record_types"] if not rt.get("coding")]
        catalog = document_cluster.current_catalog()
        if unresolved and config.RECORD_TYPE_LLM_FALLBACK and catalog is not None:
            aliases = record_type_aliases(v2_result)
            try:
                coded, usage = await resolve_with_model(
                    [
                        {
                            "name": rt["name"],
                            "aliases": aliases.get(rt["name"].lower(), []),
                        }
                        for rt in unresolved
                    ],
                    catalog,
                    q.text,
                    pipeline._call_model_async,
                    model_name=config.RECORD_TYPE_LLM_FALLBACK_MODEL or model_name,
                    location=resolved_location,
                    max_labels=config.RECORD_TYPE_LLM_FALLBACK_MAX_LABELS,
                )
            except Exception as e:  # the fallback must never fail the request
                coded, usage = {}, {}
                lg.log_struct(
                    message="Record-type model fallback failed",
                    structured_data={
                        **base_log,
                        "query_id": q.id,
                        "error_type": type(e).__name__,
            ...
            for rt in unresolved:
                rt["coding"] = [{"code": c} for c in coded.get(rt["name"].lower(), [])]
            lg.log_struct(
                message="Record-type model fallback",
                structured_data={
                    **base_log,
                    "query_id": q.id,
                    "record_types": [rt["name"] for rt in unresolved],
                    "coded": [rt["name"] for rt in unresolved if rt["coding"]],
                    "usage_metadata": usage,
                },
                severity="INFO",
            )
        still_blank = [
            rt["name"] for rt in block["record_types"] if not rt.get("coding")
        ]
        if still_blank:
            lg.log_struct(
                message="Record types uncoded after every stage",
                structured_data={
                    **base_log,
                    "query_id": q.id,
                    "record_types": still_blank,
                    "catalog_loaded": catalog is not None,
                    "json_fallback": config.RECORD_TYPE_JSON_FALLBACK,
                    "llm_fallback": config.RECORD_TYPE_LLM_FALLBACK,
                },
                severity="WARNING",
            )
    ...
    # Operational numbers stay out of the envelope (the /v1 contract): where
    # the time went and which temporal path ran are logged per request.
    lg.log_struct(
        message="Retrieval signals request timing",
        structured_data={
            **base_log,
            "endpoint": "v3" if canonical else "v2",
            "temporal_mode": temporal_mode,
            "temporal_shadow": temporal_shadow,
            "queries": len(query_results),
            "total_llm_seconds": round(total_llm_seconds, 3),
            "record_type_resolution_seconds": round(sum(cluster_seconds), 3),
        },
        severity="INFO",
    )
