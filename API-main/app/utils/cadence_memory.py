...
def aggregate_events(
    ...
    inferred windows that resolved to a formula count; explicit spans are the
    query's, not the model's, and say nothing about the concept's cadence."""
    ...
    for ev in events:
        ...
        if ev.get("basis") != "inferred":
            continue
        if ev.get("resolution") not in ("vocabulary", "formula_only"):
            continue
