...
from app.utils.temporal_canonical import window_span_seconds
...
def _codes_of(entry: dict[str, Any]) -> list[str]:
    raw = entry.get("codes")
...
def _formula_of(entry: dict[str, Any]) -> list[str]:
    formula = entry.get("formula")
    if isinstance(formula, str):
        return [formula]
    return list(formula) if isinstance(formula, list) else []


def project_temporal_by_candidate(
    v2_result: dict[str, Any], max_entries: int
) -> list[dict[str, Any]]:
    """/v3: one window per candidate concept, with the audit fields canonical
    mode produced (basis, rationale). Entries that share a candidate and a
    formula are merged (their CUIs are unioned), so a window coded by two
    vocabulary names appears once. Capped at ``max_entries`` in pipeline order.
    """
    out: list[dict[str, Any]] = []
    merged: dict[tuple[str, tuple[str, ...]], dict[str, Any]] = {}
    for intent in v2_result.get("intents", []) or []:
        for fc in intent.get("final_candidates") or []:
            candidate = str(fc.get("candidate") or "").strip()
            if not candidate:
                continue
            for t in (fc.get("retrieval_signals") or {}).get("temporal") or []:
                formula = _formula_of(t)
                key = (candidate.lower(), tuple(formula))
                codes = _codes_of(t)
                if key in merged:
                    existing = merged[key]
                    for c in codes:
                        if c not in existing["_codes"]:
                            existing["_codes"].append(c)
                    continue
    ...
    for entry in merged.values():
        codes = entry.pop("_codes")
        entry["coding"] = [{"system": "UMLS", "code": c} for c in codes]
        out.append(entry)
    return out


def project_primary_temporal(
    v2_result: dict[str, Any],
    temporal_vocab: TemporalVocab | None = None,
) -> dict[str, Any] | None:
    """/v3 primary ``temporal``: the query's explicit window when one was
    stated, else the WIDEST inferred window across candidates (a too-narrow
    primary loses documents), else the legacy projection. Falls back to
    project_temporal when the result carries no canonical fields (legacy
    mode), so the shape is always the single object /v1 and /v2 return."""
    explicit: list[tuple[float, dict[str, Any]]] = []
    inferred: list[tuple[float, dict[str, Any]]] = []
    for intent in v2_result.get("intents", []) or []:
        for t in (intent.get("retrieval_signals") or {}).get("temporal") or []:
            basis = t.get("basis")
            if basis not in ("explicit", "inferred"):
                continue
            span = window_span_seconds(t.get("window"))
            if span is None:
                continue
            entry = {
                "name": t.get("time_window"),
                "formula": _formula_of(t),
                "coding": [{"system": "UMLS", "code": c} for c in _codes_of(t)],
            }
            (explicit if basis == "explicit" else inferred).append((span, entry))
    if explicit:
        # First explicit window in pipeline order (there is normally one).
        return explicit[0][1]
    if inferred:
        return max(inferred, key=lambda p: p[0])[1]
    return project_temporal(v2_result, temporal_vocab=temporal_vocab)
