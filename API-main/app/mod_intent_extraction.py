...
class ContextualIntentPipeline:
    ...
    async def build_context_async(
        ...
        # Canonical mode (/v3): no vocabulary list. The model emits a structured
        # window per entry; the index resolves it after the call. Only a small
        # data-derived menu, a query shortlist and learned examples are injected.
        canonical = temporal_mode == TEMPORAL_MODE_CANONICAL
        if canonical:
            temporal_matching_block = self._canonical_temporal_block(
                query, expanded_query, intents
            )
        # FOLDED matching: when a vocab is set, inject the concept names so the
        # model picks a match for each temporal_signal inline. Otherwise leave
        # the block empty (the temporal_resolver handles matching downstream).
    ...
    async def run_v2_async(
        ...
        if shadow_metadata is not None:
            usage_metadata["shadow_contextual_environment"] = shadow_metadata
    ...
    # -----------------------
    def _canonical_temporal_block(
        self, query: str, expanded_query: str, intents: list[dict[str, Any]]
    ) -> str:
        """Prompt block for canonical mode: codeable-window menu (from the
        index), query shortlist (index similarity) and learned examples
        (cadence memory). Every part is derived from data and bounded by
        settings; the vocabulary list itself is never injected."""
        ...
        return build_temporal_canonical_block(
            units=list(CANONICAL_UNITS),
            menu=menu,
            shortlist=shortlist,
            examples=examples,
            default_name=(
                self._temporal_vocab.default_name()
                if self._temporal_vocab is not None
                else None
            ),
        )
    ...
    async def _temporal_shadow_compare(
        ...
        **kwargs: Any,
        ...
        shadow call's usage metadata (reported under
        usage_metadata.shadow_contextual_environment) or None if it failed."""
        try:
