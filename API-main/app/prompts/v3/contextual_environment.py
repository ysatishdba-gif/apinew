...
the type-default name from the vocabulary. No clinical cadence is authored
here: the rules say how to reason, the shape examples show only the output
format.
"""
...
Output shape (format only - the span of an inferred window comes from the candidate, never from here):
  ...
  candidate with no time wording, type default "{default_temporal_name}" -> signal "{default_temporal_name}", signal_basis type_default,
...
def build_contextual_environment_prompt_v3(
    *,
    ...
    memory at request time, and ``default_name`` is the vocabulary's own
    default temporal name (``TemporalVocab.default_name()``) so even the
    format example quotes the terminology rather than a literal.
    """
    return CONTEXTUAL_ENVIRONMENT_PROMPT_V3.format(
        ...
        default_temporal_name=default_name or "<type default>",
    )
