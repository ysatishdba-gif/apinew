...
the candidate concept, and only the disagreements are printed for a human to
sample; nothing is labelled by hand up front.
"""
...
def judge(
    ...
    items: list[dict] = []
    for row in rows:
        for w in row.get("temporal_by_candidate") or []:
            if w.get("basis") == "inferred":
                items.append(
                    {
                        "query": row["query"],
                        "candidate": w.get("candidate"),
                        "window": w.get("name") or w.get("formula"),
