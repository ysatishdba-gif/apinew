...
### Deployment checklist — values to update per environment

Everything else in this service has a working default; these are the values that differ per environment and have to be filled in (in `config/.env.<env>`, or as Cloud Run environment variables). They are marked `>>> UPDATE PER ENVIRONMENT <<<` in `app/config.py` and in every `config/.env.*` file.

| Variable | Where it points | Required |
|---|---|---|
| `PROJECT_ID` | GCP project the service runs in (Vertex AI, GCS vocabularies) | **Yes** |
| `CLUSTER_SELECTION_URL` | Cluster-selection service endpoint (Cloud Run URL including path; also the identity-token audience) | **Yes for /v2 and /v3** |
| `CLUSTER_BQ_DATASET` | BigQuery dataset holding the document-cluster tables | **Yes for /v2 and /v3** |
| `CLUSTER_BQ_PROJECT` | BigQuery project of that dataset | When it is not `PROJECT_ID` |
| `CLUSTER_BQ_LOCATION` | BigQuery location of the dataset | When it is not `US` |
| `CLUSTER_MEMBER_TABLE` / `CLUSTER_MAP_TABLE` / `CLUSTER_CLUSTER_TABLE` | Table names inside the dataset | When not `SCENARIO_6_*` |
| `CLUSTER_SET` | Cluster set name the service is queried with | When not `loinc_document_v001` |
| `TRANSACTION_SELECTION_URL` | Transaction-selection service endpoint (also its identity-token audience). When set, record types are resolved through the full pipeline below. | **Yes for the transaction pipeline** |
| `CLUSTER_ACTIVITY_TABLE` / `CLUSTER_ALL_RESULTS_TABLE` | Activity table and results table (document classes + node ids) in the same dataset | When not `SCENARIO_6_ACTIVITY_CODE_DETAILS_ARRAY` / `all_results` |
| `RECORD_TYPE_RESOLUTION_MODE` | `auto` (pipeline when the URL is set), `transactions`, or `cluster` (direct cluster → catalog) | Optional |
| `TEMPORAL_GCS_BUCKET` / `TEMPORAL_GCS_PATH` | Temporal vocabulary (falls back to the bundled file) | Optional |
| `RECORD_TYPE_GCS_BUCKET` / `RECORD_TYPE_GCS_PATH` | Record-type vocabulary for `/v1` matching and the `/v2`/`/v3` fallback | Optional |

Permissions for the service account: invoker on the cluster-selection service, BigQuery Data Viewer + Job User on the dataset, Vertex AI user, and read on the GCS vocabularies. Startup logs `Document-cluster record-type resolution configured` (with the URL, project and dataset it will use) or a `WARNING` naming the missing settings, and `Document catalog loaded` with the member count once the first request loads the tables.
...
| `CLUSTER_DOCUMENT_CATALOG_TTL_SECONDS` | No | The `MEMBER → MAP → CLUSTER` join is loaded once and kept in memory for this long; member names are matched against it in memory (no BigQuery call per request). `0` = one BigQuery query per request. Default `3600`. |
| `CLUSTER_MEMBER_MATCH_MIN_TOKEN_SCORE` | No | Abbreviation-aware token match floor: every token of the shorter name must match a token of the longer one (equal, singular/plural, prefix or contraction abbreviation — `pres.`/`prescription`, `outpt`/`outpatient` — or the initials for acronyms — `H&P`); score = matched tokens / tokens of the longer name. `0` disables. Default `0.6`. |
| `CLUSTER_ALIAS_TEXTS_MAX` | No | Extra search texts per record type sent to the cluster service: the vocabulary labels the model itself matched the record type to in the contextual-environment call (the `/v1` semantic match). `0` = the name only. Default `2`. |
| `CLUSTER_MEMBER_MATCH_MIN_SIMILARITY` | No | Character n-gram similarity floor for matching a member name the service returned to a catalog name when neither the normalised nor the punctuation-folded form is equal. `0` = exact/folded only. Default `0.75`. |
| `CLUSTER_SELECTION_MAX_WORKERS` | No | Concurrent cluster-selection calls per query (one per record type). Default `8`. |
| `RECORD_TYPE_LLM_FALLBACK` / `RECORD_TYPE_LLM_FALLBACK_MODEL` / `RECORD_TYPE_LLM_FALLBACK_MAX_LABELS` | No | Record types still uncoded after every deterministic stage are matched by the model against the cluster's document classes, in one call per request for all of them; labels resolve to CUIs through the catalog. Defaults `true` / request model / `400` (longer class lists are shortlisted by similarity). |
| `RECORD_TYPE_JSON_FALLBACK` | No | Keep the `/v1` vocabulary CUIs for a record type the cluster could not code. Default `true`. |
| `CLUSTER_MEMBER_NAME_KEYS` | No | Comma-separated field name(s) that carry member document names in the cluster-selection response (searched recursively). Default `member_name`. |
| `TAG_GCS_BUCKET` / `TAG_GCS_PATH` / `TAG_LOCAL_PATH` | No | Optional tag vocabulary (`{ name: [{"cui": ...}] }`) used by `/v2` and `/v3` to code `tags[].coding`. Not set → tags carry names and topics only (as `/v1`). |
...
- **Transaction-selection pipeline** (when `TRANSACTION_SELECTION_URL` is set): each record-type label (with the query's tag names as context) runs the standalone link-code flow — cluster selection → candidate activities of the probable documents (`CLUSTER_ACTIVITY_TABLE`) → Stage 1 activity selection by the transaction-selection service (Yes/No, batches of `TRANSACTION_BATCH_SIZE`) → documents of the selected activities → Stage 2 document selection → document classes of the selected documents (`CLUSTER_ALL_RESULTS_TABLE`) → Stage 3 class selection → the selected classes' node ids are the CUIs. The label is tried first, then the vocabulary labels the model matched it to. A record type the pipeline leaves uncoded continues through the stages below; the pipeline's stage counts are logged per record type (`Record-type transaction pipeline`). Batch answer-count mismatches and transient errors are retried as in the standalone code. Expect several transaction-selection calls (each an LLM call) per record type — this path is the slowest and the most precise.
- Record-type matching is never exact-string. Each record type is sent to the cluster service under its own label **and** under the vocabulary labels the model matched it to (`CLUSTER_ALIAS_TEXTS_MAX`), so a label such as `PRES. record` reaches the service as `Prescription Record` too. The member names the service returns — and the label and its aliases themselves — are then matched against the in-memory **document catalog** (the `MEMBER → MAP → CLUSTER` join, loaded once per process) in four steps: normalised equality (case, whitespace), folded equality (punctuation, separators), abbreviation-aware token match (`CLUSTER_MEMBER_MATCH_MIN_TOKEN_SCORE`: prefixes, contractions, plurals, acronyms, reordered qualifiers), then character n-gram similarity (`CLUSTER_MEMBER_MATCH_MIN_SIMILARITY`) for spelling variants. Nothing about what an abbreviation stands for is written in code; conventions that share no letters with the full word (`Rx`, `d/c`) are covered by the model's alias labels and the `/v1` vocabulary fallback. A request costs no BigQuery round trip. A label that names a document **class** directly (`Imaging Report`) is coded from the class label itself. Whatever is still uncoded keeps the `/v1` vocabulary CUIs, and as the last stage is matched by the model against the cluster's document classes (`RECORD_TYPE_LLM_FALLBACK`). A record type comes back with an empty `coding` only when every stage found nothing, and that is logged (`Record types uncoded after every stage`). Similarity matches are logged (`Document-cluster member names matched by similarity`); names that match nothing are logged with the catalog size. If the catalog cannot be loaded the per-request BigQuery query (case/whitespace-insensitive) is used instead. The time spent here is logged per request (`Retrieval signals request timing`).
- The contextual-environment prompt is `/v1`'s, `KNOWN RECORD TYPES` block included, so record-type **names** are generated exactly as `/v1`. Only the coding changes: the document cluster first; a name the cluster cannot code keeps the CUIs the `/v1` vocabulary match found for it (`RECORD_TYPE_JSON_FALLBACK`, default `true`), so a cluster gap never blanks a record type that `/v1` would have coded.
...
6. **`/v3/retrieval-signals`** — same request and response as `/v1`/`/v2`; only the temporal **matching** changes: the temporal vocabulary is indexed in-process instead of being pasted into the prompt, the model states each window as a structured span and code resolves it to the vocabulary entry (exact, else the narrowest containing window). Queries with no time expression get a window inferred per candidate from its clinical nature, resolved the same way.
...
**Contract: identical to `/v1` and `/v2`.** Same request body (with `/v2`'s optional `actions`), same envelope, same per-query block (`id`, `text`, `record_types[{name, coding}]`, `temporal{name, formula, coding}`, `tags`), same `details`. Nothing is added. Record-type and temporal **names are generated exactly as `/v1`** (the same contextual-environment Task 1 and Section A/B instructions). What changes is only how a temporal signal is **matched** to the vocabulary:
...
| Output `temporal` | a vocabulary name, its CUIs, its formula (first non-default window in pipeline order) | **the same** — a vocabulary name, its CUIs, its formula, chosen by the same projection |
