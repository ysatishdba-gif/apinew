...
# ===========================================================================
# >>> DEPLOYMENT SETTINGS — transaction-selection pipeline (record types) <<<
#
# When TRANSACTION_SELECTION_URL is set, each record type is resolved the way
# the standalone pipeline does it: cluster selection -> activities (BigQuery)
# -> activity selection -> related documents -> document selection ->
# document classes (BigQuery results table) -> class selection; the selected
# classes' node ids are the CUIs. Without it, the direct cluster -> catalog
# path is used. RECORD_TYPE_RESOLUTION_MODE: auto (pipeline when the URL is
# set) | transactions | cluster.
#
#   TRANSACTION_SELECTION_URL   the transaction-selection service endpoint
#                               (also its identity-token audience)
#   CLUSTER_ACTIVITY_TABLE      activity table (default
#                               SCENARIO_6_ACTIVITY_CODE_DETAILS_ARRAY)
#   CLUSTER_ALL_RESULTS_TABLE   results table with document classes (default
#                               all_results); its columns below
# ===========================================================================
TRANSACTION_SELECTION_URL = os.getenv("TRANSACTION_SELECTION_URL", "").strip()
RECORD_TYPE_RESOLUTION_MODE = (
    os.getenv("RECORD_TYPE_RESOLUTION_MODE", "auto").strip().lower() or "auto"
)
TRANSACTION_SELECTION_MODEL = (
    os.getenv("TRANSACTION_SELECTION_MODEL", "gemini-2.5-flash").strip()
    or "gemini-2.5-flash"
)
CLUSTER_ACTIVITY_TABLE = (
    os.getenv(
        "CLUSTER_ACTIVITY_TABLE", "SCENARIO_6_ACTIVITY_CODE_DETAILS_ARRAY"
    ).strip()
    or "SCENARIO_6_ACTIVITY_CODE_DETAILS_ARRAY"
)
CLUSTER_ALL_RESULTS_TABLE = (
    os.getenv("CLUSTER_ALL_RESULTS_TABLE", "all_results").strip() or "all_results"
)
CLUSTER_ALL_RESULTS_DOC_COLUMN = (
    os.getenv("CLUSTER_ALL_RESULTS_DOC_COLUMN", "document_type").strip()
    or "document_type"
)
CLUSTER_ALL_RESULTS_CLASS_COLUMN = (
    os.getenv("CLUSTER_ALL_RESULTS_CLASS_COLUMN", "document_class").strip()
    or "document_class"
)
CLUSTER_ALL_RESULTS_CODE_COLUMN = (
    os.getenv("CLUSTER_ALL_RESULTS_CODE_COLUMN", "node_id").strip() or "node_id"
)
# Batching / concurrency / retries for the transaction-selection calls.
TRANSACTION_BATCH_SIZE = max(1, _env_int("TRANSACTION_BATCH_SIZE", 25))
TRANSACTION_MAX_WORKERS = max(1, _env_int("TRANSACTION_MAX_WORKERS", 8))
